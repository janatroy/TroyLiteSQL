...
namespace FtpClient
{
    ...
    public class FtpClient
    {
        #region "CONSTRUCTORS"
         ...
         public bool Upload(string localFilename, [System.Runtime.InteropServices.Optional] string targetFilename) // ERROR: Optional parameters aren't supported in Cstring targetFilename)
         {
             //1. check source
         ...
         /// <returns></returns>
         public bool Upload(FileInfo fi, [System.Runtime.InteropServices.Optional] string targetFilename) // ERROR: Optional parameters aren't supported in Cstring targetFilename)
         {
         ...
         public bool Download(string sourceFilename, string localFilename,[System.Runtime.InteropServices.Optional] bool  PermitOverwrite) // ERROR: Optional parameters aren't supported in Cbool PermitOverwrite)
         {
         ...
         public bool Download(FTPfileInfo file, string localFilename, [System.Runtime.InteropServices.Optional] bool  PermitOverwrite) // ERROR: Optional parameters aren't supported in Cbool PermitOverwrite)
         {
             return this.Download(file.FullName, localFilename, PermitOverwrite);
         }
        
         //Another version taking FtpFileInfo and FileInfo
         public bool Download(FTPfileInfo file, FileInfo localFI,[System.Runtime.InteropServices.Optional] bool  PermitOverwrite)
         {
             return this.Download(file.FullName, localFI, PermitOverwrite);
         }
        
         //Version taking string/FileInfo
         public bool Download(string sourceFilename, FileInfo targetFI,[System.Runtime.InteropServices.Optional] bool  PermitOverwrite)
         {
             //1. check target
        ...
        private string GetDirectory([System.Runtime.InteropServices.Optional] string directory)
        {
            string URI = null;
...
//return directory, ensure it ends with /
            get { return _currentDirectory + (string)(_currentDirectory.EndsWith("/") ? "" : "/"); }
            set {
    ...
    public class FTPfileInfo
    {
        ...
        public FTPfileInfo(string line, string path)
        {
            ...
            if (m == null) {
                ...
                _path = path;
                _size = Convert.ToDouble( m.Groups["size"].Value);
    ...
    public class FTPdirectory : List<FTPfileInfo>
    {
        ...
        //{
        //    foreach (string line in dir.Replace(Constants.vbLf, "").Split((char)Constants.vbCr))
        //    {
        //        parse
        //        if (!string.IsNullOrEmpty(line)) this.Add(new FTPfileInfo(line, path));
        //    }
        //}
        ...
        public FTPdirectory GetFiles([System.Runtime.InteropServices.Optional] string ext) // ERROR: Optional parameters aren't supported in Cstring ext)
        {
